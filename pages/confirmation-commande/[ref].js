...
export default function ConfirmationCommande() {
  ...
  const [checkModuleInfo45, setCheckModuleInfo45] = useState({});
  const [checkModuleInfo60, setCheckModuleInfo60] = useState({});
    ...
    await fetch("/api/confirmationOrder", requestOptions)
      ...
      .then((result) => {
        if (result?.succes) {
          ...
          if (result?.check_45_module_info) {
            setCheckModuleInfo45(result?.check_45_module_info);
          }
          if (result?.check_60_module_info) {
            setCheckModuleInfo60(result?.check_60_module_info);
          }
  ...
  return (
    <>
      ...
      <div className="md:max-w-7xl mx-auto my-5 space-y-5">
