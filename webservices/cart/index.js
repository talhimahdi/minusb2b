...
const cart = {
  ...
  getCartIdBySecureKey(key) {
    return fetch(
      `${config.apiUrl}carts/?display=[id]
  ...
  getCart(id) {
    return fetch(
      `${config.apiUrl}carts/${id}?ws_key=${config.apiKey}&${config.dataType}`
    )
      .then((res) => res.json())
      .then((d) => d.cart);
  },
  ...
  getCartItemData(item) {
    return new Promise((resolve, reject) => {
      getCombination(item.id_product_attribute)
        .then((d) => {
          getOptionValues(d.associations.product_option_values).then((data) => {
            products.getProductInfo(item.id_product).then((product_d) => {
              let img =
                typeof d.associations.images === "undefined"
                  ? [{ id: product_d[0].id_default_image }]
                  : d.associations.images.slice();
              resolve([
                {
                  images: img,
